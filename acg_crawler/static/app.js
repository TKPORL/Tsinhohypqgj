/* ==========================================================================
   ...
   【硬性规则】新增任何 fetch 都必须带 .catch，否则又会"点了没反应、也不报错"。

   历史事故：用户遇到过「爬取控制」页正常，但「爬取结果 / 历史记录 / 导出文件 /
   数据管理」点进去一片空白，只有一个背景图。原因是 21 个 fetch 里有 13 个没写
   失败分支 —— 接口一报错，.then 不执行、内容不渲染，但面板已经切过去了，
   于是就是一片空白且不给任何提示。2026-09-26（v20, commit e2ee6aa）已全部补齐。

   标准写法：
       fetch("/api/xxx")
           .then(checkOk)                              // 4xx/5xx 直接抛错
           .then(function(r) { return r.json(); })
           .then(function(data) { ... })
           .catch(function(err) {
               showPanelError("容器id", err, "/api/xxx");   // 列表类容器
               // 或 showGlobalError(_failText(err, "/api/xxx"));  // 数字/按钮类
           });

   下面 5 个工具函数就是给这个用的，直接用，别重复造：
       checkOk(resp)                         状态码校验
       _failText(err, url)                   异常 → 人话
       showPanelError(容器id, err, url)      面板内红色错误块
       showGlobalError(text)                 页顶红色兜底条
       _esc(s)                               HTML 转义

   注意：另一个"面板空白"的原因在 templates/index.html —— 少一个 </div> 会让面板
   被嵌进 panel-crawl 内部，父级 display:none 时子孙全隐藏。改 index.html 后请
   数一遍 <div> 与 </div> 是否配平。详见【项目全解】说明文档 §0.6。
   ========================================================================== */
