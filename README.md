...
### 2026-09-26 (v23) 请求失败兜底：接口出错时显示原因，不再静默空白
- 背景：历史上出现过「爬取控制」正常、其余面板点进去一片空白。**真因不是图片**：一是 `index.html` 少一个 `</div>` 导致面板被嵌套（已修），二是 `app.js` 21 个 `fetch` 里 13 个没写失败分支 —— 接口一报错，内容不渲染但面板已切过去，就是空白且无提示
- 本次补齐：`app.js` 顶部新增 `checkOk` / `_failText` / `showPanelError` / `showGlobalError` / `_esc` 五个工具函数；13 个 `fetch` 全部补 `.catch`；另加 `window` 级 `error` + `unhandledrejection` 兜底条
- 效果：接口挂掉时面板内显示「加载失败：<原因>　[/api/接口名]」，页顶红条显示未捕获错误；进度接口断掉会停轮询并改状态为「连接中断」
- 验证：无头 Chrome 三场景（接口 abort / 返回 500 / 正常）—— 前两者各面板均有明确报错、无空白，正常时错误块数量 0、控制台无报错
- ⚠️ **以后新增任何 `fetch` 都必须带 `.catch`**，详见 `【项目全解】说明文档.md` §0.6

### 2026-09-26 (v22) 界面改版：全站换「深色 · 粉紫」+ 8 项优化
- 视觉：Origin 深色编辑风 → 深色 · 粉紫。修掉三处中文排版错配（中文标签被加 `letter-spacing` 致字距撑散、标题用无中文字形的衬线字体、满屏英文装饰）；主色改粉红 `#e056a0`；平台分组头由满幅大色块改细条；去掉 Google Fonts 外链（导出 HTML 离线可用）
- 优化：日期默认值改动态（原写死 `2026-09-01` 已过期）、站点列表改后端下发（新增 `/api/sites`）、结果页工具栏拆两行、新增总览条（新增 `/api/stats`）、批次删除入口并入数据管理页、历史表格窄屏隐藏 3 列、「重新下载图片」移到数据管理页
- 修复：批量条未选中时没藏干净；卡片图片网格 `auto-fill` 留空轨道致单图只占半张卡宽（改 `auto-fit`）；平台分组头写了手型光标但只有按钮能点
- 验证：无头 Chrome 5 面板截图，控制台无报错，390px 横向溢出 0px；导出 HTML 实测 1280/390px 均 0 溢出
